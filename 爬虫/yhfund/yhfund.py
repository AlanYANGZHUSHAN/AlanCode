...
class Yhfund:
	...
	def get_report_title_one_page(self,href,fund_name):
		...
		for k in range(1,11):
			try:
				temp_href = temp_page.find_element_by_xpath(TITLE_REPORT_PATH%k).get_attribute('href')
				temp_date = temp_page.find_element_by_xpath(TITLE_REPORT_PATH%k+'/span[2]').text
				if self.start_date <= datetime.strptime(temp_date, "%Y/%m/%d") and self.end_date >= datetime.strptime(temp_date, "%Y/%m/%d"):
					temp_title = temp_page.find_element_by_xpath(TITLE_REPORT_PATH%k+'/span[1]').text
	...
	def del_same_title(self):
		...
		df = df.drop_duplicates(['title url'],keep='first')
		return df
